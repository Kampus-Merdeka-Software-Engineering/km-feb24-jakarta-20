import datasets from './data.json' assert { type: 'json' };
    ...
    for (let i = 0; i < datasets.length; i++) {
        ...
        // Untuk Line Chart
        if (transactionQtyMonths.filter((item) => item.id === dataset.transaction_date.split('-')[1]).length > 0) {
        ...
        if (averageQtyAndVallueStores.filter((item) => item.id === dataset.store_id).length > 0) {
        ...
        if (averageTransactionQtyPerStore.filter((item) => item.id === dataset.store_id).length > 0) {
            averageTransactionQtyPerStore = averageTransactionQtyPerStore.map((item) => {
                if (item.id === dataset.store_id) {
                    return {
                        ...item,
                        value: item.value + 1,
        ...
        if (averageQtyAndVallueProductType.filter((item) => item.id === dataset.product_type).length > 0) {
            averageQtyAndVallueProductType = averageQtyAndVallueProductType.map((item) => {
    ...
    document.querySelector('#avgQtySalesPerMonth').innerHTML = (avgQtySalesPerMonth / 6).toFixed(2);
    document.querySelector('#averageQtyPerTransaction').innerHTML = (totalUnitPrice / totalTransactionQty).toFixed(2);
    document.querySelector('#averageSpendingPerTransaction').innerHTML = "$" + averageSpendingPerTransaction.toFixed(2);
    document.querySelector('#averageSpendingPerTransactionQty').innerHTML = "$" + (averageSpendingPerTransaction / totalTransactionQty).toFixed(2);
    ...
    for (let i = 0; i < averageQtyAndVallueProductType.length; i++) {
        ...
        <tr class="mid">
            ...
            <td>${(productType.transaction_qty * productType.price).toFixed(2)}</td>
        </tr>`;
    }
    productTable.innerHTML += `
...
const dataPieChart = getDataPieChart(averageTransactionQtyPerStore, totalTransactionQty);
...
document.querySelector("#filterButton").addEventListener("click", (event) => {
    ...
    const newDatasets = datasets.filter((dataset) => {
        const dateTransaction = new Date(dataset.transaction_date).setHours(0, 0, 0, 0);
        ...
        } else if (valueStore !== "all" && valueStartDate !== "" && valueEndDate === "") {
